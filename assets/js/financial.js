...
function addCharge() {
    ...
    const serviceName = selectedOption ? selectedOption.getAttribute('data-service') || '' : '';
    ...
    if (!serviceName) {
        showNotification('Service name not found', 'error');
        return;
    }
    ...
    // Medication calculation works: 0% doctor fee, 100% clinic fee
...
function generateReceiptHTML() {
    ...
    let servicesHTML = '';
    if (calculationData.charges.length > 0) {
        servicesHTML = calculationData.charges.map(charge =>
            `<div class="service-item">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                    <span style="font-weight: bold; color: #2563eb;">• ${charge.service}</span>
                    <span style="font-weight: bold; color: #059669;">RM ${charge.amount.toFixed(2)}</span>
                </div>
            </div>`
        ).join('');
    } else {
        servicesHTML = '<div class="service-item">• General Consultation</div>';
    }
    ...
    if (calculationData.otherCharges.length > 0) {
        otherChargesHTML = `
            <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #e5e7eb;">
                <div style="font-weight: bold; color: #7c3aed; margin-bottom: 8px;">📋 Additional Charges</div>
                ${calculationData.otherCharges.map(charge =>
                    `<div class="service-item">
            ...
            <div class="receipt-container">
                ...
                <div class="receipt-body">
                    <div class="info-row">
                        ...
                        <span class="info-value">${customerName || 'Walk-in Patient'}</span>
                    </div>
                    ${terminalInvoice ?
                        `<div class="info-row">
                            <span class="info-label">Terminal Invoice:</span>
                            <span class="info-value">${terminalInvoice}</span>
                        </div>` : ''
                    }
                    ...
                    <div class="totals-section">
                        <div class="total-row final">
                            ...
                            <span>RM ${(calculationData.totalDoctorFee + calculationData.totalClinicFee).toFixed(2)}</span>
                        </div>
                    </div>

                    <div class="payment-method">
                        💳 Payment Method: ${calculationData.paymentMethod}
                    </div>
                </div>
